...
<%
	...
	ResultSet rs=null;
	String sql="select * from product2024 where productId=?";
	...
	if(rs.next()){
		...
		String instock=rs.getString("unitinstock");
	    ...
	    <tr>
	      ...
	      <td >
	      	<select name="category" class="drop">
	      		<option value="it 제품" <%if(cate.equals("10")){ %>selected<% } %>>it 제품</option>
	      		<option value="주방제품" <%if(cate.equals("20")){ %>selected<% } %>>주방제품</option>
	      		<option value="전자제품" <%if(cate.equals("30")){ %>selected<% } %>>전자제품</option>
	      		<option value="일반 잡화" <%if(cate.equals("40")){ %>selected<% } %>>일반 잡화</option>
	      	</select>
	      </td>
	    </tr>
	    <tr>
